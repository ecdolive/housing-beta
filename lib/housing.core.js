...
input_edit = debounce(function(x){
  ...
  var value = $(x).val();
  if(value != values[name].display){ // if value changed
    // update cookie
    //if(val && parseFloat(val) != 0){ // does not allow for zero values
    if(value){ // allows for zero values
      Cookies.set(name,value,{expires:3650});
      if(isLocalHost()) console.log('Cookie: set "' + name + '" to "' + value + '";');
    }
    else {
      Cookies.remove(name);
      if(isLocalHost()) console.log('Cookie: remove "' + name + '";');
    }
    calculate(id,value);
  }
},100);
...
location_select = function(x){
  ...
  if($(x).data()){
    ...
    if(locs[y]){
      ...
      // refresh_inputs();
      calculate();
      Cookies.set('location',y,{expires:3650});
      if(isLocalHost()) console.log('Cookie: set "location" to "' + y + '";');
      // DELETE TAX RATE COOKIE
    }
  }
};
