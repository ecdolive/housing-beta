...
location_select = function(x){
  ...
  if($(x).data()){
    ...
    ga('send','event','location','select',loc.id);
    debug.log("\tga('send','event','location','select','" + loc.id + "')");
  }
  else {
    ga('send','event','location','select','unknown');
    debug.log("\tga('send','event','location','select','unknown')");
  }
};
