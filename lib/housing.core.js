...
init_values = function(){
  ...
  values.inputs.price.display = cookies.price || "250000"; // Purchase price
  values.inputs.down.display = cookies.down || "10%"; // Down payment
  values.inputs.closing.display = cookies.closing || "4%"; // Closing costs
  values.inputs.tax.display = cookies.tax || "0.87%"; // Effective tax rate
  values.inputs.hoi.display = cookies.hoi || null; // Yearly homeowners insurance -> calc_hoi()
  values.inputs.flood.display = cookies.flood || null; // Yearly flood insurance -> calc_flood()
  values.inputs.month.display = cookies.month || $('#month').val(); // Purchase month
  values.inputs.year.display = cookies.year || $('#year').val(); // Purchase year
  values.inputs.term.display = cookies.term || "30"; // Loan term
  values.inputs.interest.display = cookies.interest || "4.5%"; // Yearly interest rate
  values.inputs.pmi.display = cookies.pmi || "1.15%"; // Yearly PMI rate
  values.inputs.hoa.display = cookies.hoa || "0"; // Monthly HOA dues
  values.inputs.extra.display = cookies.extra || "0"; // Extra monthly payment
  values.inputs.appreciation.display = cookies.appreciation || "2%"; // Yearly appreciation rate
  values.inputs.rent.display = cookies.rent || "1275"; // Monthly rent comparison
  calculate();
  amortization_graph();
  display_all();
};
