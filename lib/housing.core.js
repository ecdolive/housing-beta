...
cookies_get = function(){
  ...
  if(isLocalHost()){
    ...
    // cookies.location = "san_francisco";
    // cookies.month = 9;
    // cookies.year = 1998;
  ...
  $.each(cookies,function(i,v){
    if(isJsonString(v)){
      cookies_update_from_old_version(i,v);
    }
    else {
      // console.log('\t' + i + ': ' + v + '; isJsonString = false; leave cookie alone...');
    }
  });
};
...
cookies_update_from_old_version = function(i,v){
  // console.log('\t' + i + ': ' + v + '; isJsonString = true; replace old cookie...');
  var old = JSON.parse(v);
  if(old.id == 'e4a' || old.id == 'e4z'){
    cookies_remove(i);
    cookies_set('upfront', old.val);
  }
  else if(old.id == 'e7'){
    cookies_remove(i);
    cookies_set('month', old.val);
  }
  else if(old.id == 'e8'){
    cookies_remove(i);
    cookies_set('year', old.val);
  }
  else if(old.id == 'e9'){
    cookies_remove(i);
    cookies_set('term', old.val);
  }
  else if(old.id == 'e10' || old.id == 'e11' || old.id == 'e16'){
    cookies_set(i, old.val + '%');
  }
  else if(old.id == 'e14' || old.id == 'e15' || old.id == 'e18'){
    cookies_remove(i);
  }
  else {
    cookies_set(i, old.val);
