...
init_values = function(){
  ...
  // set_king(king);
      values.inputs.down.display = loc.data.down || cookies.down || "10%";
      values.inputs.closing.display = loc.data.closing || cookies.closing || "4%"; king = "closing";
      // values.inputs.upfront.display = null;
...
input_edit = debounce(function(x){
  ...
  if(value != values.inputs[name].display){
    ...
    //   }
    // }
        if(id == 'tax' && loc.id != 'custom'){
          loc.id = 'custom';
          // location_set($('[data-value="custom"]')); // NOTE: for some reason calling location_set here breaks the values.inputs.tax object
        }
    calculate();
    amortization_graph();
    display_all();
