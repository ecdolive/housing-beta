...
input_edit = debounce(function(x){
  ...
  if(value != values[name].display){ // if value changed, update cookie and recalculate
    if(value) cookies_set(name,value); // if value in input field, set cookie
    else cookies_remove(name); // if nothing in input field, remove cookie
    calculate(id,value);
  }
},100);
...
location_select = function(x){
  ...
  if($(x).data()){
    ...
    if(locs[y]){
      ...
      // TODO 1c: reset all inputs back to saved || default
      $.each(locs[y],function(i,v){
        // if(i != 'name' && !values[i].saved){
        if(i != 'name'){
          // TODO 1a: use values[*].location to determine whether or not to show saved value or selected location value
          // TODO 2a [PREFERRED]: or get rid of values[*].location altogether (inputs_fill) and only handle changing location values via location_select()
          // console.log('\t\t'+i+' -> '+v);
          // values[i].location = v;
          // values[i].display = v;
          switch(i){
          ...
          cookies_remove(i); // TODO 1b: above todo may require this line to be deleted
        }
      });
