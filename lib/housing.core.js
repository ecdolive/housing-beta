...
amortization_click = function(){
  ...
  at += `<div class="labels">
      <row>
        ...
        <cell>Balance</cell>
        <cell>Equity</cell>
        <cell>Unequitable costs</cell>
        <cell>Recoverable payments</cell>
        <cell>Total payments</cell>
      </row>
    </div>`;
  at += '<div class="rows">';
  $.each(values.monthly_amortization,function(i,v){
    ...
    // r = m × [ ( 1 + i)1/m - 1 ]
    var disabled = i == 0 ? 'style="display: none"' : '';
    // <cell>${ v.date } (${ v.month }mo)</cell>
    at += `<row>
        <cell id="date-${ i }">${ v.date } (${ i == 0 ? 'origination' : v.month + 'mo' })</cell>
        <cell><input id="monthly-appreciation-${ i }" value="${ (values.outputs.monthly_nominal_appreciation / 12 * 100).toFixed(5) }%" ${ disabled }></cell>
        <cell id="value-${ i }">${ v.value.toFixed(0) }</cell>
        <cell><input id="extra-payment-${ i }" value="${ values.inputs.extra.number.toFixed(0) }" ${ disabled }></cell>
        <cell id="balance-${ i }">${ v.balance.toFixed(0) }</cell>
        <cell id="equity-${ i }">${ v.equity.toFixed(0) }</cell>
        <cell id="unequitable-${ i }">${ v.unequitable_costs.toFixed(0) }</cell>
        <cell id="recoverable-${ i }">${ Math.round(v.equity/v.payments.total*100) }%</cell>
        <cell id="total-${ i }">${ v.payments.total.toFixed(0) }</cell>
      </row>`;
      // <cell id="recoverable-${ i }" style="color: rgb(0,${ ((Math.round((v.equity/v.payments.total)*100)/100*255*1.5)-127.5) },0)">${ (Math.round((v.equity/v.payments.total)*100)) }%</cell>
  });
  at += '</div>';
  at += '</div>';
  at += '</div>';
  $('#amortization-table').append(cl).append(at);
  $('.rows input').on('keyup',function(){
    amortization_edit(this);
  });
  $('#amortization-table').addClass('show');
  calculate();
  amortization_write();
  ga('send','event','amortization','click','open');
  debug.log("\tga('send','event','amortization','click','open')");
};
amortization_close = function(){
  $('#amortization-table').empty().removeClass('show');
  ga('send','event','amortization','click','close');
  debug.log("\tga('send','event','amortization','click','close')");
};
amortization_edit = debounce(function(x){
  var id = $(x).attr('id'); // id of input being edited
  debug.log('amortization_edit(#'+id+')');
...
amortization_write = function(){
  $.each(values.monthly_amortization,function(i,v){
    $('#value-' + i).html(v.value.toFixed(0));
    $('#balance-' + i).html(v.balance.toFixed(0));
    $('#equity-' + i).html(v.equity.toFixed(0));
    $('#unequitable-' + i).html(v.unequitable_costs.toFixed(0));
    $('#recoverable-' + i).html(Math.round(v.equity / v.payments.total * 100) + '%');
    $('#total-' + i).html(v.payments.total.toFixed(0));
  });
  $.each(values.monthly_amortization,function(i,v){
    if(v.balance.toFixed(0) < 0){
      $('#date-' + (parseInt(i) + 1)).parent().css('display','none');
    }
  });
};
function Debug(){}
...
copyToClipboard = function(){
  ...
  // var fullUrl = baseUrl + '?v=1' + `,${ values.inputs.month.display },${ values.inputs.year.display },${ values.inputs.price.display },${ values.inputs.down.display },${ values.inputs.closing.display },${ values.inputs.pmi.display },${ values.inputs.interest.display },${ values.inputs.term.display },${ values.inputs.tax.display },${ values.inputs.hoi.display },${ values.inputs.flood.display },${ values.inputs.hoa.display },${ values.inputs.extra.display },${ values.inputs.appreciation.display }`;
  // $temp.val(fullUrl).select();
  // document.execCommand('copy');
  // $temp.remove();
  var $temp = $('<input>');
  $('body').append($temp);
  var fullUrl = $('#bookmarklet').val();
  $temp.val(fullUrl).select();
  document.execCommand('copy');
  $temp.remove();
  $('#bookmarklet').select();
...
parseURL = function(){
  if(get_param('v')){
    var p = get_param('v').split(',');
    url.version = p[0];
    if(url.version == 1){
      // ?v=1,6,2018,405000,20250,962,0.41p,4.5p,30,0.7318p,1008,0,0,100,2%
      // ?v=version,month,year,price,down,closing,pmi,interest,term,tax,hoi,flood,hoa,extra,appreciation
...
init_values = function(){
  ...
  values.inputs.rent = {};
  // 1. try to set from cookie, 2. set using the standard default
  values.inputs.price.display = url.price || cookies.price || "250000"; // Purchase price
  values.inputs.down.display = url.down || cookies.down || "10%"; // Down payment
  values.inputs.closing.display = url.closing || cookies.closing || "4%"; // Closing costs
  values.inputs.tax.display = url.tax || cookies.tax || "0.87%"; // Effective tax rate
  values.inputs.hoi.display = url.hoi || cookies.hoi || null; // Yearly homeowners insurance -> calc_hoi()
  values.inputs.flood.display = url.flood || cookies.flood || null; // Yearly flood insurance -> calc_flood()
  values.inputs.month.display = url.month || cookies.month || $('#month').val(); // Purchase month
  values.inputs.year.display = url.year || cookies.year || $('#year').val(); // Purchase year
  values.inputs.term.display = url.term || cookies.term || "30"; // Loan term
  values.inputs.interest.display = url.interest || cookies.interest || "4.5%"; // Yearly interest rate
  values.inputs.pmi.display = url.pmi || cookies.pmi || "1.15%"; // Yearly PMI rate
  values.inputs.hoa.display = url.hoa || cookies.hoa || "0"; // Monthly HOA dues
  values.inputs.extra.display = url.extra || cookies.extra || "0"; // Extra monthly payment
  values.inputs.appreciation.display = url.appreciation || cookies.appreciation || "2%"; // Yearly appreciation rate
  values.inputs.rent.display = url.rent || cookies.rent || "1275"; // Monthly rent comparison
  calculate();
  amortization_graph();
  display_all();
};
