...
amoritization_click = function(){
  ...
  var at = '<div><div class="atable">';
  at += '<row><cell>Date</cell><cell>Home value</cell><cell>Balance</cell><cell>Equity</cell><cell>Unequitable cost</cell><cell>Recoverable payments (from sale)</cell><cell>Total payments</cell></row>';
  $.each(values.amoritization,function(i,v){
    at += '<row><cell>' + v.date + ' (year ' + v.year + ')</cell><cell>' + v.value + '</cell><cell>' + v.balance + '</cell><cell>' + v.equity + '</cell><cell>' + v.cost_sale + '</cell><cell style="color: rgb(0,' + ((Math.round((v.equity/v.payments.total)*100)/100*255*1.5)-127.5) + ',0)">' + (Math.round((v.equity/v.payments.total)*100)) + '%</cell><cell>' + v.payments.total + '</cell></row>';
  });
  at += '</div></div>';
...
init_values = function(){
  ...
  switch(king){
    case "upfront": send.upfront = loc.data.upfront || cookies.upfront || null; break;
    case "down": send.down = loc.data.down || cookies.down || "10%"; break;
    case "closing":
    default: send.closing = loc.data.closing || cookies.closing || "4%"; king = "closing"; break;
  }
  switch(queen){
    case "upfront": send.upfront = loc.data.upfront || cookies.upfront || null; break;
    case "closing": send.closing = loc.data.closing || cookies.closing || "4%"; break;
    case "down":
    default: send.down = loc.data.down || cookies.down || "10%"; queen = 'down'; break;
  }
  switch(pawn){
    case "down": break;
    case "closing": break;
  ...
  send.interest = loc.data.interest || cookies.interest || "4.5%"; // Yearly interest rate
...
location_set = function(x){
  ...
  loc.data = locations[loc.id] || {};
  if(loc.id == 'custom'){
    loc.data.tax = cookies.tax || send.tax;
...
set_king = function(x){
  ...
  if(king == x){
    ...
    if(x == 'upfront'){
      if(king == x){}
      else if(king == 'down'){ king = x; queen = 'down'; pawn = 'closing'; send.closing = null; }
      else if(king == 'closing'){ king = x; queen = 'closing'; pawn = 'down'; send.down = null; }
      else { king = x; queen = 'closing'; pawn = 'down'; send.down = null; }
    }
    else if(x == 'down'){
      if(king == x){}
      else if(king == 'upfront'){ king = x; queen = 'upfront'; pawn = 'closing'; send.closing = null; }
      else if(king == 'closing'){ king = x; queen = 'closing'; pawn = 'upfront'; send.upfront = null; }
      else { king = x; queen = 'closing'; pawn = 'upfront'; send.upfront = null; }
    }
    else if(x == 'closing'){
      if(king == x){}
      else if(king == 'upfront'){ king = x; queen = 'upfront'; pawn = 'down'; send.down = null; }
      else if(king == 'down'){ king = x; queen = 'down'; pawn = 'upfront'; send.upfront = null; }
      else { king = x; queen = 'down'; pawn = 'upfront'; send.upfront = null; }
    }
    cookies_remove(pawn);
