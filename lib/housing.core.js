...
cookies_get = function(){
  ...
  if(isLocalHost()){
    ...
    // cookies.location = '{"id":"none","val":"san_francisco"}';
    // cookies.month = '{"id":"e7","val":9}';
    // cookies.year = '{"id":"e8","val":1998}';
    // cookies.price = '{"id":"e1a","val":299999}';
    // cookies.down = '{"id":"e4a","val":29999}';
    // cookies.closing = '{"id":"e3a","val":"4.9%"}';
    // cookies.loan = '{"id":"e9","val":29}';
    // cookies.interest = '{"id":"e10","val":3.99}';
    // cookies.pmi = '{"id":"e11","val":1.99}';
    // cookies.hoa = '{"id":"e12","val":99}';
    // cookies.extra = '{"id":"e13","val":99}';
    // cookies.hoi = '{"id":"e5","val":1999}';
    // cookies.flood = '{"id":"e6","val":999}';
    // cookies.tax = '{"id":"e14","val":0.99}';
    // cookies.appreciation = '{"id":"e16","val":1.99}';
    // cookies.rent = '{"id":"e17","val":1299}';
  }
  else {
    cookies = Cookies.get();
  }
...
input_edit = debounce(function(x){
  var id = $(x).attr('id');
  var dc = $(x).attr('data-cookie');
  var val = $(x).val();
  if(val != values.calc[dc]){ // if value changed
...
values_set = function(){
  ...
  $.each(cookies,function(i,v){
    ...
    if(i != '_ga' && i != '_gid' && i != '_gat'){ // only add to saved if the cookie is not an analytics cookie
      if(i == 'down'){
        values.saved[i] = v;
      }
      else {
