...
location_select = function(x){
  ...
  if($(x).data()){
    ...
    ga('send','event','change location',loc.id);
    debug.log("ga('send','event','change location','" + loc.id + "')");
  }
  else {
    ga('send','event','change location','unknown');
    debug.log("ga('send','event','change location','unknown')");
  }
};
