...
defaults = function(){
  ...
  e.e3b = parsePercent(e.e3a) * e.e1a; // Closing costs alt
  e.e3c = e.e3b; // Closing costs int
  e.e4a = e.e2c + e.e3c; // Paid at closing
  e.e4b = makePercent(e.e4a,e.e1a); // Paid at closing alt
  e.e4c = e.e4a; // Paid at closing int
  e.e4z = e.e4c; // Paid at closing
...
cookies = function(){
  ...
  // closing
    if(c.closing){
      var p = JSON.parse(c.closing);
      console.log('\tclosing: ' + c.closing);
      e.e3a = p.val;
      delete c.closing;
    }
  ...
  // down
    if(c.down){
      var p = JSON.parse(c.down);
      // code chunk A start
      if(p.id == 'e4a' || p.id == 'e4z'){
        // if user defined total paid at closing...
        console.log('\tdown: ' + c.down + ' (user defined total paid at closing)');
        e.e4a = p.val;
        e.e4z = p.val;
        e.e4b = makePercent(p.val,e.e1a);
        e.e4c = p.val;
        $('#e4a, #e4z').val(p.val);
        $('#e4b').text(e.e4b);
        ...
        console.log('\tdown: ' + c.down + ' (user defined down payment)');
        // so far, only handles %, not int
        e.e2a = p.val;
        e.e2b = Math.round(parsePercent(p.val) * e.e1a);
        e.e2c = e.e2b;
        $('#e2a').val(p.val);
        $('#e2b').text(e.e2b);
...
handleClosing = function(id,val){
  // code chunk A start
  if(id == 'e4a' || id == 'e4z'){
    // if user defined total paid at closing...
    console.log('\t\tuser defined total paid at closing');
    // so far, only handles int, not %
    e.e4a = e.e4z = val;
    if(id == 'e4a') $('#e4z').val(val);
    else $('#e4a').val(val);
    e.e4b = makePercent(val,e.e1a);
    $('#e4b').text(e.e4b);
    e.e4c = val;
  }
  else if(id == 'e2a'){
    // if user defined down payment...
    console.log('\t\tuser defined down payment');
    // so far, only handles %, not int
    e.e2a = val;
    $('#e2a').val(val);
    e.e2b = Math.round(parsePercent(val) * e.e1a);
    $('#e2b').text(e.e2b);
    e.e2c = e.e2b;
    // calculate actual "paid at closing" based on "closing costs" and "down payment"
  ...
  // code chunk A end
  console.log('handleClosing(' + id + ', ' + val + ')');
};

handlePrice = function(id,val){
  e.e1a = e.e1z = val;
  if(id == 'e1a'){
    $('#e1z').val(val);
  }
  ...
  console.log('handlePrice(' + id + ', ' + val + ')');
};

calculate = function(id,val){
  console.log('calculate()');
  if(id && val){
    // handle multi-direcional inputs
    if(id == 'e1a' || id == 'e1z'){
      handlePrice(id,val);
    }
    else if(id == 'e2a' || id == 'e4a' || id == 'e4z'){
      handleClosing(id,val);
    }
    else {
      e[id] = val;
    }
  ...
  u.u92 = Math.round(u.u11m + (e.e5 / 12) + (e.e6 / 12) + ((((e.e1a * 0.4) / 1000) * e.e14) / 12) + ((((e.e1a * 0.4) / 1000) * e.e15) / 12) + e.e12); // Monthly taxes, insurance, dues
