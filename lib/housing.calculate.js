calculate = function(month,year,price,upfront,closing,down,term,interest,pmi,hoa,extra,hoi,flood,tax,appreciation,rent){
  ...
  c.monthly_amoritization = {};
  var _ma = c.monthly_amoritization;

  var date_start = new Date(_i.year.display,_i.month.display);

  // month 0 // TODO: cleanup this object structure
  _ma[0] = {};
  _ma[0].month = 0;
  ...
  _ma[0].cost_sale = _ma[0].payments.total - _ma[0].equity;
  _ma[0].cost_rent = 0;
  ...
  for(var month = 1; month <= final_month; month++){
    ...
    var in_year = Math.ceil(_ma[month].month / 12);
    var month_number = _ma[month].month - (12 * (in_year - 1));
    _ma[month].value = Math.round(_a[in_year-1].value + ((_a[in_year].value - _a[in_year - 1].value) / 12 * month_number));
    _ma[month].balance = Math.max(0, Math.round(-FV(_o.monthly_interest_rate, month, -_o.monthly_principle_interest_with_extra, _o.loan)));
    _ma[month].equity = _ma[month].value - _ma[month].balance;
    // need to fix interest, p&i calculations to work correctly for final partial months
    //interest[month] = Math.round(balance[last] > 0 ? (month * 12 * _o.monthly_principle_interest_with_extra) - (_o.loan - balance[month]) : interest[last]);
    //pi[month] = _o.years_to_pay_off >= month ? Math.round(_o.monthly_principle_interest_with_extra * 12) : 0;
    _ma[month].interest = {};
    _ma[month].pi = {};
    if(month == final_month && final_month != _o.years_to_pay_off * 12){
      ...
      var remainder = 1 - (final_month - (_o.years_to_pay_off * 12)); // monthly remainder, i.e. days as decimal // console.log('remainder: ' + remainder + '; remainder months: ' + (remainder * 12));
      _ma[month].interest.total = Math.round(_o.total_interest);
      _ma[month].pi.monthly = Math.round(remainder * _o.monthly_principle_interest_with_extra);
    }
    else {
      // All months, except the final month if it's a partial month
      //console.log('month ' + month + ': regular');
      _ma[month].interest.total = Math.round(month * _o.monthly_principle_interest_with_extra) - (_o.loan - _ma[month].balance);
      _ma[month].pi.monthly = Math.round(_o.monthly_principle_interest_with_extra);
    }
    _ma[month].pi.total = Math.round(_ma[last].pi.total + _ma[month].pi.monthly);
    _ma[month].interest.monthly = Math.round(_ma[month].interest.total - _ma[last].interest.total);
    _ma[month].principle = {};
    _ma[month].principle.total = Math.round((_ma[month].pi.monthly * month) - _ma[month].interest.total);
    _ma[month].principle.monthly = Math.round(_ma[month].principle.total - _ma[last].principle.total);
    _ma[month].pmi = {};
    _ma[month].pmi.yearly = _ma[month].balance > price * .8 ? Math.round(_o.loan * _i.pmi.percent) : 0; // pmi calculated against original loan amount
    ...
    _ma[month].tax = {};
    _ma[month].tax.monthly = Math.round(_ma[month].value * _i.tax.percent / 12); // tax calculated against actual value each month (not last tax assessment)
    _ma[month].tax.total = Math.round(_ma[month].tax.monthly + _ma[last].tax.total);
    _ma[month].hoa = {};
    ...
    _ma[month].cost_sale = _ma[month].payments.total - _ma[month].equity;
    _ma[month].cost_rent = _i.rent.number * month;
    _ma[month].ratio = Math.round((_ma[month].payments.total / _ma[month].equity) * 100) / 100;
    _ma[month].cost_avg = {};
    _ma[month].cost_avg.monthly = Math.round(_ma[month].cost_sale / month);
