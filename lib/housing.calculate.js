/*
TODO: set cookie on manual input edit
TODO: need reset to default option for editable inputs and selects
TODO: what all should really change when location is selected?
*/
calc_flood = function(){
...
calculate = function(id,val){
  if(id && val){
    ...
    if(id == 'e1a' || id == 'e1z'){
      ...
      var d = $('#' + id).data('cookie');
      values[d].display = val;
    }
  }
...
inputs_fill = function(){
  ...
  $.each(cookies,function(i,v){
    if(i != '_ga' && i != '_gid' && i != '_gat'){ // ignore analytics cookies
      // console.log('values.'+i+'.saved = '+v+';');
      if(i != 'none') // handle an erroneous 'none' cookie that was previously possible
        values[i].saved = v;
    }
  });
