...
//     }
//   });
//   // set all display values, which ultimately is what gets calculated // TODO 2b: can probably get rid of all values.*.location below
//   values.location.display = values.location.location || values.location.saved || values.location.default; // Location
//     // set any location defined values (values.*.location) // TODO 2c: can probably get rid of this chunk since location_select() (triggered in edit_location() below and on selection of new location) will handle location defined values
//     $.each(locations[values.location.display],function(i,v){
//       if(i != 'name') values[i].location = v;
//     });
//   values.price.display = values.price.location || values.price.saved || values.price.default; // Purchase price
//   values.upfront.display = values.upfront.location || values.upfront.saved || values.upfront.default; // Total at closing
//   values.down.display = values.down.location || values.down.saved || values.down.default; // Down payment
...
//     master_set('down');
//   }
//   values.closing.display = values.closing.location || values.closing.saved || values.closing.default; // Closing costs
//   values.tax.display = values.tax.location || values.tax.saved || values.tax.default; // Effective tax rate
//   values.hoi.display = values.hoi.location || values.hoi.saved || calc_hoi(); // Yearly homeowners insurance
//   values.flood.display = values.flood.location || values.flood.saved || calc_flood(); // Yearly flood insurance
//   values.month.display = values.month.location || values.month.saved || $('#e7').val(); // Purchase month
//   values.year.display = values.year.location || values.year.saved || $('#e8').val(); // Purchase year
//   values.term.display = values.term.location || values.term.saved || values.term.default; // Loan term
//   values.interest.display = values.interest.location || values.interest.saved || values.interest.default; // Yearly interest rate
//   values.pmi.display = values.pmi.location || values.pmi.saved || values.pmi.default; // Yearly PMI rate
//   values.hoa.display = values.hoa.location || values.hoa.saved || values.hoa.default; // Monthly HOA dues
//   values.extra.display = values.extra.location || values.extra.saved || values.extra.default; // Extra monthly payment
//   values.appreciation.display = values.appreciation.location || values.appreciation.saved || values.appreciation.default; // Yearly appreciation rate
//   values.rent.display = values.rent.location || values.rent.saved || values.rent.default; // Monthly rent comparison
