/*
TODO:
- on changePrice, homeowners insurance and flood insurance needs to update if they have not been explicitly set by the user
- need to reset to default for editable inputs and selects
...
inputs_fill = function(){
  ...
  values.calc = {};
  // location
    values.calc.location = values.saved.location || values.defaults.location;
    $('#locations > li[data-value="' + values.calc.location + '"]').addClass('selected');
  // price
    values.calc.price = values.saved.price || values.defaults.price; // Home purchase price
    //e.e1z = values.calc.price; // Home purchase price
  // closing
    values.calc.closing = values.saved.closing || values.defaults.closing; // Closing costs
    if(isPercentage(values.calc.closing)){
      e.e3b = parsePercent(values.calc.closing) * values.calc.price;
      e.e3c = e.e3b;
    }
    else {
      e.e3b = makePercent(values.calc.closing,values.calc.price);
      e.e3c = parseInt(values.calc.closing);
    }
  // down
    if(values.saved.down){
      var p = values.saved.down;
      if(p.id == 'e4a' || p.id == 'e4z'){
        // if user defined total paid at closing...
        e.e4a = p.val;
        if(isPercentage(e.e4a)){
          e.e4b = parsePercent(e.e4a) * values.calc.price;
          e.e4c = e.e4b;
        }
        else {
          e.e4b = makePercent(e.e4a,values.calc.price);
          e.e4c = parseInt(e.e4a);
        }
        e.e4z = e.e4c;
        // calculate down payment
        e.e2c = Math.round(e.e4c - e.e3c);
        e.e2a = makePercent(e.e2c,values.calc.price);
        e.e2b = e.e2c;
        slave_set('down');
      }
      else if(p.id == 'e2a'){
        ...
        if(isPercentage(e.e2a)){
          e.e2b = Math.round(parsePercent(e.e2a) * values.calc.price);
          e.e2c = e.e2b;
        }
        else {
          e.e2b = makePercent(e.e2a,values.calc.price);
          e.e2c = parseInt(e.e2a);
        }
        // calculate actual "paid at closing" based on "closing costs" and "down payment"
        e.e4c = e.e2c + e.e3c;
        e.e4b = makePercent(e.e4c,values.calc.price);
        e.e4a = e.e4c;
        e.e4z = e.e4c;
        slave_set('total');
      }
      ...
      e.e2a = values.defaults.down.val; // Down payment
      e.e2b = parsePercent(e.e2a) * values.calc.price; // Down payment alt
  ...
  // tax
    if(values.saved.tax){
      values.calc.tax = values.saved.tax;
    }
    else if(values.saved.location && locs[values.saved.location]){
      values.calc.tax = locs[values.saved.location].tax_rate;
    }
    else {
      values.calc.tax = values.defaults.tax
    }
  // all others
    values.calc.hoi = values.saved.hoi || Math.round((values.calc.price / 1000) * 3.5); // Yearly homeowners insurance
    values.calc.flood = values.saved.flood || Math.round((values.calc.price / 1000) * 1.8); // Yearly flood insurance
    values.calc.month = values.saved.month || $('#e7').val(); // Purchase month
    values.calc.year = values.saved.year || $('#e8').val(); // Purchase year
    values.calc.loan = values.saved.loan || values.defaults.loan; // Loan term
    values.calc.interest = values.saved.interest || values.defaults.interest; // Yearly interest rate
    values.calc.pmi = values.saved.pmi || values.defaults.pmi; // Yearly PMI rate
    values.calc.hoa = values.saved.hoa || values.defaults.hoa; // Monthly HOA dues
    values.calc.extra = values.saved.extra || values.defaults.extra; // Extra monthly payment
    values.calc.appreciation = values.saved.appreciation || values.defaults.appreciation; // Yearly appreciation rate
    values.calc.rent = values.saved.rent || values.defaults.rent; // Monthly rent comparison
  refresh_inputs();
};
...
change_to = function(val){
  ...
  var percent;
  if(isPercentage(val)){
    number = Math.round(parsePercent(val) * values.calc.price);
    percent = val;
    opposite = number;
  }
  else {
    number = parseInt(val);
    percent = makePercent(val,values.calc.price);
    opposite = percent;
  }
  return {
    'value': val,
...
changeDown = function(id,val){
  ...
  // if user defined total paid at closing...
    if(id == 'e4a'){
      ...
      e.e2c = e.e4c - e.e3c;
      e.e2a = makePercent(e.e2c,values.calc.price); $('#e2a').val(e.e2a);
  ...
  // if user defined down payment...
      ...
      e.e4b = makePercent(e.e4c,values.calc.price); $('#e4b').text(e.e4b);
...
changeClosing = function(id,val){
  ...
  var i = change_to(val);
  values.calc.closing = i.value;
  e.e3b = i.opposite; $('#e3b').text(e.e3b);
  e.e3c = i.number;
  if(slave == 'total'){
    e.e4c = e.e2c + e.e3c;
    e.e4a = e.e4c; $('#e4a').val(e.e4a);
    e.e4b = makePercent(e.e4a,values.calc.price); $('#e4b').text(e.e4b);
    e.e4z = e.e4c; $('#e4z').val(e.e4z);
  }
  else {
    e.e2c = e.e4c - e.e3c;
...
changePrice = function(id,val){ // DONE
  ...
  if(slave == 'down') { // if total at closing is the master input
    ...
    if(isPercentage(e.e4a)){ // if total at closing is defined as %
      ...
      e.e4b = Math.round(parsePercent(e.e4a) * values.calc.price);
      e.e4c = e.e4b;
      $('#e4b').text(e.e4b);
    }
    ...
    // closing
    if(isPercentage(values.calc.closing)){ // if closing costs is defined as %
      console.log('\tclosing costs is percentage');
      e.e3b = Math.round(parsePercent(values.calc.closing) * values.calc.price);
      e.e3c = e.e3b;
      $('#e3b').text(e.e3b);
    }
  ...
  if(!values.saved.hoi){
    values.calc.hoi = (values.calc.price / 1000) * 3.5; // Yearly homeowners insurance
    $('#e5').val(values.calc.hoi);
  }
  if(!values.saved.flood){
    values.calc.flood = (values.calc.price / 1000) * 1.8; // Yearly flood insurance
    $('#e6').val(values.calc.flood);
  }
};
...
calculate = function(id,val){
  ...
  if(id && val){
    ...
    if(id == 'e1a' || id == 'e1z'){
      ...
      //e[id] = val;
      var dc = $('#' + id).data('cookie');
      values.calc[dc] = val;
    }
  }

  u.u0 = values.calc.price - e.e2c; // Loan amount
  u.u10 = (values.calc.interest / 12) / 100; // Monthly interest rate
  u.u11y = u.u0 * (values.calc.pmi / 100); // Yearly PMI
  u.u11m = e.e2c / u.u0 < .2 ? Math.round(parseFloat(u.u11y / 12)) : 0; // Monthly PMI
  u.u12 = values.calc.hoa * 12; // Yearly HOA dues
  u.u17 = values.calc.rent * 12; // Yearly rent
  u.u91 = Math.round(parseFloat(PMT(u.u10,values.calc.loan * 12,-u.u0))); // Monthly principal & interest
  var tax_monthly = (values.calc.price * (values.calc.tax / 100)) / 12;
  u.u92 = Math.round(tax_monthly + u.u11m + (values.calc.hoi / 12) + (values.calc.flood / 12) + values.calc.hoa); // Monthly taxes, insurance, dues
  u.u93 = Math.round(parseFloat(u.u91) + parseFloat(values.calc.extra)); // Monthly mortgage payment
  u.u94a = Math.round(parseFloat(u.u93) + parseFloat(u.u92) + parseFloat(values.calc.hoa)); // Total monthly payments
...
refresh_inputs = function(){
  $('#e1a').val(values.calc.price);
  $('#e1z').val(values.calc.price);
  $('#e2a').val(e.e2a);
  $('#e2b').text(e.e2b);
  $('#e3a').val(values.calc.closing);
  $('#e3b').text(e.e3b);
  $('#e4a').val(e.e4a);
  $('#e4b').text(e.e4b);
  $('#e4z').val(e.e4z);
  $('#e5').val(values.calc.hoi);
  $('#e6').val(values.calc.flood);
  $('#e7').val(values.calc.month);
  $('#e8').val(values.calc.year);
  $('#e9').val(values.calc.loan);
  $('#e10').val(values.calc.interest);
  $('#e11').val(values.calc.pmi);
  $('#e12').val(values.calc.hoa);
  $('#e13').val(values.calc.extra);
  $('#e14').val(values.calc.tax);
  $('#e16').val(values.calc.appreciation);
  $('#e17').val(values.calc.rent);
};
