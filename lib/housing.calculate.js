calculate = function(month,year,price,upfront,closing,down,term,interest,pmi,hoa,extra,hoi,flood,tax,appreciation,rent){
  ...
  for(var month = 1; month <= final_month; month++){
    ...
    _ma[month].value = _a[in_year-1].value + ((_a[in_year].value - _a[in_year - 1].value) / 12 * month_number);
    ...
    _ma[month].pmi = {};
    _ma[month].pmi.yearly = _ma[month].balance > price * .8 ? (_o.loan * _i.pmi.percent) : 0; // pmi calculated against original loan amount
