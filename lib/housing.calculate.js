...
ui_set = function(){
  ...
  // location
    if(values.saved.location){
      $('#locations > li[data-value="' + values.saved.location + '"]').addClass('selected');
    }
    else {
      $('#locations > li[data-value="' + values.defaults.location + '"]').addClass('selected');
    }
  // price
    if(values.saved.price){
      var p = values.saved.price;
      e.e1a = p;
      e.e1z = p;
      $('#e1a, #e1z').val(p.val);
    }
    else {
      e.e1a = values.defaults.price; // Home purchase price
      e.e1z = e.e1a; // Home purchase price
    }
  // closing
    if(values.saved.closing){
      var p = values.saved.closing;
      e.e3a = p.val;
    }
    else {
      e.e3a = values.defaults.closing; // Closing costs
    }
  ...
  // all others
    ...
    if(values.saved.location && locs[values.saved.location].tax_rate) e.e14 = locs[values.saved.location].tax_rate;
    else e.e14 = values.defaults.tax // Tax rate
