...
inputs_fill = function(){
  ...
  // location
    var loc = values.saved.location || values.defaults.location;
    $('#locations > li[data-value="' + loc + '"]').addClass('selected');
  // price
    e.e1a = values.saved.price || values.defaults.price; // Home purchase price
    e.e1z = e.e1a; // Home purchase price
  // closing
    e.e3a = values.saved.closing || values.defaults.closing; // Closing costs
  ...
  // tax
    if(values.saved.tax){
      e.e14 = values.saved.tax;
    }
    else if(values.saved.location && locs[values.saved.location]){
      e.e14 = locs[values.saved.location].tax_rate;
    }
    else {
      e.e14 = values.defaults.tax
    }
  // all others
    e.e5 = values.saved.hoi || Math.round((e.e1a / 1000) * 3.5); // Yearly homeowners insurance
    e.e6 = values.saved.flood || Math.round((e.e1a / 1000) * 1.8); // Yearly flood insurance
    e.e7 = values.saved.month || $('#e7').val(); // Purchase month
    e.e8 = values.saved.year || $('#e8').val(); // Purchase year
    e.e9 = values.saved.loan || values.defaults.loan; // Loan term
    e.e10 = values.saved.interest || values.defaults.interest; // Yearly interest rate
    e.e11 = values.saved.pmi || values.defaults.pmi; // Yearly PMI rate
    e.e12 = values.saved.hoa || values.defaults.hoa; // Monthly HOA dues
    e.e13 = values.saved.extra || values.defaults.extra; // Extra monthly payment
    e.e16 = values.saved.appreciation || values.defaults.appreciation; // Yearly appreciation rate
    e.e17 = values.saved.rent || values.defaults.rent; // Monthly rent comparison
  refresh_inputs();
};
...
calculate = function(id,val){
  ...
  */
  var tax_monthly = (e.e1a * (e.e14 / 100)) / 12;
  u.u92 = Math.round(tax_monthly + u.u11m + (e.e5 / 12) + (e.e6 / 12) + e.e12); // Monthly taxes, insurance, dues
