calculate = function(){
  ...
  for(var year = 1; year <= final_year; year++){
    ...
    _a[year].pmi = {};
    // _a[year].pmi.yearly = _a[year].balance > values.inputs.price * .8 ? Math.round(_a[year].balance * _i.pmi.percent) : 0; // pmi calculated against loan balance
    _a[year].pmi.yearly = _a[year].balance > values.inputs.price * .8 ? Math.round(_o.loan * _i.pmi.percent) : 0; // pmi calculated against original loan amount
  ...
  for(var month = 1; month <= final_month; month++){
    ...
    // TODO: create listeners on amortization inputs to run calculate()
    // TODO: long term: cookie the amortization input fields/values
    // TODO: look for cookie, then input values, then global input value for each monthly amortization option // $('#monthly-appreciation-rate-' + month).value() || _o.monthly_nominal_appreciation
    _ma[month].value = _ma[last].value + (_ma[last].value * (_o.monthly_nominal_appreciation) / 12);
    _ma[month].balance = Math.max(0, -FV(_o.monthly_interest_rate, month, -_o.monthly_principle_interest_with_extra, _o.loan));
    ...
    if(month == final_month && final_month != _o.years_to_pay_off * 12){
      ...
      _ma[month].interest.total = _o.total_interest;
      _ma[month].pi.monthly = remainder * _o.monthly_principle_interest_with_extra;
    }
    else {
      // All months, except the final month if it's a partial month
      //console.log('month ' + month + ': regular');
      _ma[month].interest.total = (month * _o.monthly_principle_interest_with_extra) - (_o.loan - _ma[month].balance);
      _ma[month].pi.monthly = _o.monthly_principle_interest_with_extra;
    }
    ...
    _ma[month].pmi = {};
    _ma[month].pmi.yearly = _ma[month].balance > values.inputs.price * .8 ? (_o.loan * _i.pmi.percent) : 0; // pmi calculated against original loan amount
    ...
    _ma[month].cost_avg = {};
    _ma[month].cost_avg.monthly = _ma[month].cost_sale / month;
